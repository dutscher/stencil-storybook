      ...
      prepareForInline: (storyFn, storyData) => {
        class Story extends React.Component {
          ...
          render() {
            ...
            const prism = document.querySelector('#anchor--' + storyData.id + ' .prismjs .language-jsx');
            if (prism) {
              prism.innerHTML = Prism.highlight(renderCode({ ...storyData.args, docsView: true }), Prism.languages.jsx, 'jsx');
            }
...
export function setupStory(stencilSettings, storySettings) {
  const settings = {
    ...
    tplCompositionWithActionLogs: (args) => html `${settings.tpl.bind()({ ...args, docsView: true })}${settings.customEvents
...
export function setupVariant(Story, args = {}, title = '') {
  ...
  const litHtmlLogs = (variantVargs) => html `${Story.tpl({ ...Variant.args, ...variantVargs })}${Story.customEvents
